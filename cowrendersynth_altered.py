...
def generate_tless_realsamples(datasetPath, sampleIds, objid="22", imD=128, crop=True, maskStr="mask_visibl", cropDim=70,justScaleImages=False, ScaledSize=128, makeNDC=True):
    ...
    objPath = datasetPath+"/train/0000" + str(objid).zfill(2) + "/"
    ...
    mesh = trimesh.load(datasetPath+"/models/obj_0000" + objid.zfill(2) + ".ply")
    cx1=0
    cx2=400
    if crop:
        cx1=cx1+cropDim
    ...
    for a in range(imCT):
        ...
        c1 = cv2.imread(objPath + "rgb/" + str(imId).zfill(6) + ".jpg")[cx1:cx2,cx1:cx2]
        m1 = cv2.imread(objPath + maskStr + '/' + str(imId).zfill(6) + "_000000.png")[cx1:cx2,cx1:cx2][:, :, 0]
        #print(objPath + maskStr + str(imId).zfill(6) + "_000000.png")
        if imD < c1.shape[0]:
            ...
            # cv2.imwrite("nhap/" + str(a) + "_target.jpg", target_images[a].numpy())
            # cv2.imwrite("nhap/" + str(a) + "_mask.jpg", target_silhouettes[a].numpy())
...
def generate_bop_realsamples(datasetPath, objid="22", imD=128, crop=True, maskStr="mask", cropDim=70,
    ...
    for a11 in range(imCT):
        ...
        if dataset == 'tless':  ## tless is special cause in each image we have 19 objects stead of just 1 object as ruapc
            rgbPath = objPath + "rgb/" + str(imId).zfill(6) + ".jpg"
            c1 = cv2.imread(objPath + "rgb/" + str(imId).zfill(6) + ".jpg")  # image shape (540, 720, 3) 
            m1 = cv2.imread(objPath + maskStr + "/" + str(imId).zfill(6) + "_000000.png")
            # ##use cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) cos the mask has 3 channels not 1
            # m1 =  cv2.cvtColor(cv2.imread(objPath + maskStr + "/" + str(imId).zfill(6) + "_000000.png"), cv2.COLOR_BGR2GRAY)[...,np.newaxis] ##mask shape (540, 720, 3)
            #print('c1.shape, m1.shape', m1.shape)
            #print("image path", objPath + "rgb/" + str(imId).zfill(6) + ".jpg")
            # cv2.imwrite("nhap/" + str(a11) + "_target.jpg", c1)
            # cv2.imwrite("nhap/" + str(a11) + "_mask.jpg", m1)
        else:
            rgbPath = objPath + "rgb/" + str(imId).zfill(6) + ".png"
            ...
            #print('c1.shape, m1.shape',c1.shape, m1.shape)
        ...
        target_silhouettes[a11] = torch.from_numpy(
            cv2.resize(squareMask1, (maxB, maxB), cv2.INTER_NEAREST).astype("float32") / 255)
        # cv2.imwrite("nhap/" + str(a11) + "_target.jpg", target_images[a11].numpy())
        # cv2.imwrite("nhap/" + str(a11) + "_mask.jpg", target_silhouettes[a11].numpy())
        if background:
            bgScale = int(maxB * squareBG1.shape[0] / squareRGB1.shape[0])
            if bgScale % 2 == 0:
                bgScale = bgScale - 1
...
def generate_cow_rendersWithRT(  ## so it is same as generate_cow_renders just in reutrn it has R,T 
