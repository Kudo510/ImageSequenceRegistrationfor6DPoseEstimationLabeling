...
# def ADD(verts, gtR1, gtT1, R1, T1):  ## ask him again about the function where did get the form?
...
##? what surface features, normals, for?


# refinement=False
# subSurfaccePointsScaled=surfacePointsScaled[np.random.choice(surfacePointsScaled.shape[0], 5000)].copy()  ##? when to use .copy?
...
for path in (file_list[:1281]):
    ...
    if True:
        ...
        if not useSurfEval: ## our case
            ...
            R2, T2, in2 = pnp(ep3d, ep2d, camMat, itr=500, reperr=2)
...
# file_name1 = str(objid) + "pred_R.txt"
...
# # Define the file name
# file_name2 = str(objid) + "pred_t.txt"
# # Open the file in write mode
...
## ? see it loads only cnn but not nerf - how did they infer 2D-3D corres? 