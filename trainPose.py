...
data_nerf = torch.load(nerfID+"/nerflatestFine.pth")   ## load here to get the nerf with mlp for color, density and features. but during the nerf training we only optimzed for color and density, so now we load it for our pose estimator - we optimize for the mlp feaures here (freeze/not use any mlps for density and colors)
### really cool cos usually we just need to initilize a new nerf model but here we can still load the old model as well. cool idea
...
## dm what n1 used for here? I cannot see anywhere it'S used
...
n_iter = int(60000*batch_size/target_images.shape[0]) ##target_images.shape[0] is num_view/number of images that we render from 3D model
...
for iterate in range(n_iter):
  for train_data, indices in sdf_loader:
    ...
    if iteration%100==0 and iteration>1:
        ...
        if not onlyNerf: ## OnlyNerf=False
    ...
    if not onlyNerf:# onlyNerf=False -out case
        if True:
            g1=1
    ...
    ##until 282: to get batch_cameras
    ...
    ## so until 353: to cal fullNegVec
    add_input_samples=False
    if negativeSampling: #negativeSampling=True
        if not rayFreeze or len(batch_idx1)==0: #rayFreeze=False
          ...
          if sampleNegativeOnce: #sampleNegativeOnce=True
              loopCT=20
          else:
              loopCT=1
          ...
          if fullNegVec.shape[0]==0:
            if os.path.exists(expID+"/negVec.npy"):
              ...
              for a in range(loopCT): #loopCT=20
                  with torch.no_grad():
    ...
    if not onlyNerf: ##onlyNerf=False -our case
     ...
     ### all lines to 424 are about compute loss for our model
    err5=0
    intraLoss=0
    intraE1=0
    ...
    ## so all below saved feat, mask, rendfeat .jpg in the folder poseEst
